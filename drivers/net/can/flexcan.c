	...
	switch (ioctl_num) {
		...
		case FLEXCAN_IOCTL_WRITE:
			...
			if(settings_bitrate || settings_mode)	{
				...
				if(err)	{
					// printk("%s.%d: %s error when try to start the module\n", fimx6d.name, dev_num, __func__);
					dev_dbg("%s.%d: %s error when try to start the module\n", fimx6d.name, dev_num, __func__);
				}
			}
		...
		case FLEXCAN_IOCTL_WR_RD:
			...
			// dev_dbg("%s.%d: %s copy from user: DLC 0x%02x, ID %#08x, DATA = 0x%02x%02x%02x%02x 0x%02x%02x%02x%02x\n", 
...
static int flexcan_char_open(struct inode *i, struct file *file)
{
	...
	if(fimx6d.f_dev[dev_num].is_open)	{
		return -EBUSY;
		dev_dbg("%s.%d: %s file was opened early\n", fimx6d.name, dev_num, __func__);
	}
	dev_dbg("%s.%d: %s file open success\n", fimx6d.name, dev_num, __func__);
...
static ssize_t flexcan_proc_read(struct file *file, char *buf, size_t count, loff_t *ppos) 
{
	...
	int res;
	dev_dbg("%s.%d: %s count = %d, buf_length = %d, *ppos = %d\n", fimx6d.name, dev_num, __func__, count, buf_length, (int) *ppos);

	if(*ppos >= buf_length) 	{
		dev_dbg("%s.%d: %s End of File\n", fimx6d.name, dev_num, __func__);
		return 0;
	}
...
static int flexcan_proc_dir_init(void) 
{
	...
	if(NULL == fimx6d.dev_proc_dir) {
		ret = -ENOENT;
		dev_dbg("%s: %s can't create directory /proc/%s\n", fimx6d.name, __func__, NAME_DIR);
		goto err_dir;
	}
...
static int flexcan_proc_file_init(u8 dev_num) 
{
	...
	dev_dbg("%s.%d: %s /proc/%s/%s installed\n", fimx6d.name, dev_num, __func__, NAME_DIR, fimx6d.f_dev[dev_num].name);
...
static unsigned int flexcan_start_transmit(u8 dev_num)
{
	...
	dev_dbg("%s.%d: %s have to send: %#08x %#02x [0x%02x%02x%02x%02x,0x%02x%02x%02x%02x]\n", fimx6d.name, dev_num, __func__, 
		cf.can_id, cf.can_dlc, cf.data[0], cf.data[1], cf.data[2], cf.data[3], cf.data[4], cf.data[5], cf.data[6], cf.data[7]);
...
static void do_bus_err(struct can_frame *cf, u32 reg_esr, u8 dev_num)
{
	...
	if (reg_esr & FLEXCAN_ESR_BIT1_ERR) {
		dev_dbg("%s.%d: %s BIT1_ERR irq\n", fimx6d.name, dev_num, __func__);
		cf->data[2] |= CAN_ERR_PROT_BIT1;
		tx_errors = 1;
	}
	if (reg_esr & FLEXCAN_ESR_BIT0_ERR) {
		dev_dbg("%s.%d: %s BIT0_ERR irq\n", fimx6d.name, dev_num, __func__);
		cf->data[2] |= CAN_ERR_PROT_BIT0;
		tx_errors = 1;
	}
	if (reg_esr & FLEXCAN_ESR_ACK_ERR) {
		dev_dbg("%s.%d: %s ACK_ERR irq\n", fimx6d.name, dev_num, __func__);
		cf->can_id |= CAN_ERR_ACK;
		cf->data[3] |= CAN_ERR_PROT_LOC_ACK;
		tx_errors = 1;
	}
	if (reg_esr & FLEXCAN_ESR_CRC_ERR) {
		dev_dbg("%s.%d: %s CRC_ERR irq\n", fimx6d.name, dev_num, __func__);
		cf->data[2] |= CAN_ERR_PROT_BIT;
		cf->data[3] |= CAN_ERR_PROT_LOC_CRC_SEQ;
		rx_errors = 1;
	}
	if (reg_esr & FLEXCAN_ESR_FRM_ERR) {
		dev_dbg("%s.%d: %s FRM_ERR irq\n", fimx6d.name, dev_num, __func__);
		cf->data[2] |= CAN_ERR_PROT_FORM;
		rx_errors = 1;
	}
	if (reg_esr & FLEXCAN_ESR_STF_ERR) {
		dev_dbg("%s.%d: %s STF_ERR irq\n", fimx6d.name, dev_num, __func__);
...
static void do_state(struct can_frame *cf, u8 dev_num, enum can_state new_state)
{
	...
	dev_dbg("%s.%d: %s get berr counter\n", fimx6d.name, dev_num, __func__);
	...
	case CAN_STATE_ERROR_ACTIVE:
		...
		if (new_state >= CAN_STATE_ERROR_WARNING &&
			...
			dev_dbg("%s.%d: %s Error Warning IRQ\n", fimx6d.name, dev_num, __func__);
	...
	case CAN_STATE_ERROR_WARNING:	/* fallthrough */
		...
		if (new_state >= CAN_STATE_ERROR_PASSIVE &&
		    new_state <= CAN_STATE_BUS_OFF) {
			dev_dbg("%s.%d: %s Error Passive IRQ\n", fimx6d.name, dev_num, __func__);
	...
	case CAN_STATE_BUS_OFF:
		dev_dbg("%s.%d: %s BUG! hardware recovered automatically from BUS_OFF\n", fimx6d.name, dev_num, __func__);
		break;
	default:
		break;
	}
...
static irqreturn_t flexcan_irq(int irq, void *dev_id)
{
	...
	dev_dbg("%s.%d: %s interrupt request, reg_iflag1 = %#08x\n", fimx6d.name, dev_num, __func__, reg_iflag1);
	...
	if (reg_iflag1 & FLEXCAN_IFLAG_DEFAULT)	{
		...
		dev_dbg("%s.%d: read buffer contain data: CTRL %#08x, ID %#08x, DATA0 %#08x, DATA1 %#08x\n", 
	...
	if (reg_iflag1 & (1 << FLEXCAN_TX_BUF_ID)) {			/* transmission complete interrupt */
		...
		dev_dbg("%s.%d: transmit buffer was contain data: CTRL %#08x, ID %#08x, DATA0 %#08x, DATA1 %#08x\n", 
		 			fimx6d.name, dev_num, mb->can_ctrl, mb->can_id, mb->data[0], mb->data[1]);
	}
...
static int flexcan_chip_start(const u8 dev_num)
{
	...
	/* enable module */
	dev_dbg("%s.%d: %s enable module\n", fimx6d.name, dev_num, __func__);
	flexcan_chip_enable(fimx6d.f_dev[dev_num].f_base);

	/* soft reset */
	dev_dbg("%s.%d: %s soft reset\n", fimx6d.name, dev_num, __func__);
	writel(FLEXCAN_MCR_SOFTRST, &regs->mcr);
	udelay(10);

	reg_mcr = readl(&regs->mcr);
	if (reg_mcr & FLEXCAN_MCR_SOFTRST) {
		dev_dbg("%s.%d: %s failed to softreset can module, mcr=0x%08x\n", fimx6d.name, dev_num, __func__, reg_mcr);
		err = -ENODEV;
		goto out;
	}

	dev_dbg("%s.%d: %s set bittiming\n", fimx6d.name, dev_num, __func__);
	...
	dev_dbg("%s.%d: %s writing mcr=0x%08x\n", fimx6d.name, dev_num, __func__, reg_mcr);
	...
	/* print chip status */
	dev_dbg("%s.%d: %s reading mcr=0x%08x ctrl=0x%08x\n", fimx6d.name, dev_num, __func__, readl(&regs->mcr), readl(&regs->ctrl));

	dev_dbg("%s.%d: %s registereng /proc file\n", fimx6d.name, dev_num, __func__);
	flexcan_proc_file_init(dev_num);

	return 0;

 out:
 	dev_dbg("%s.%d: %s chip disable\n", fimx6d.name, dev_num, __func__);
...
static void flexcan_chip_stop(const u8 dev_num)
{
	...
	dev_dbg("%s.%d: %s was call with dev_num = %d\n", fimx6d.name, dev_num, __func__, dev_num);
	
	regs = fimx6d.f_dev[dev_num].f_base;
	dev_dbg("%s.%d: %s unregistereng /proc file\n", fimx6d.name, dev_num, __func__);
...
static int flexcan_net_open(const u8 dev_num)
{
	...
	if (err)	{
		dev_dbg("%s.%d: %s request irq %d failed with err = %d\n", fimx6d.name, dev_num, __func__, irq, err);
		goto out;
	}
	else 	{
		dev_dbg("%s.%d: %s request irq %d success\n", fimx6d.name, dev_num, __func__, irq);
	}
	/* start chip and queuing */
	dev_dbg("%s.%d: %s chip start\n", fimx6d.name, dev_num, __func__);
	err = flexcan_chip_start(dev_num);
	if (err)	{
		goto out;
	}
 ...
 out:
 	dev_dbg("%s.%d: %s goto out. clk disable\n", fimx6d.name, dev_num, __func__);
...
static int __devinit register_flexcandev(const u8 dev_num)
{
	...
	if (!(reg & FLEXCAN_MCR_FEN)) {
		dev_dbg("%s.%d: %s Could not enable RX FIFO, unsupported core\n", fimx6d.name, dev_num, __func__);
		err = -ENODEV;
		goto out;
	}
...
static int __devexit flexcan_remove(struct platform_device *pdev)
{
	...
	dev_dbg("%s.%d: %s remove flexcan driver start\n", fimx6d.name, pdev->id, __func__);

	flexcan_net_close(pdev->id);

	// flexcan_proc_file_exit(pdev->id);
	dev_dbg("%s.%d: %s device destroy\n", fimx6d.name, pdev->id, __func__);
	...
	dev_dbg("%s.%d: %s iounmap\n", fimx6d.name, pdev->id, __func__);
	iounmap(fimx6d.f_dev[pdev->id].f_base);

	mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	dev_dbg("%s.%d: %s release mem region\n", fimx6d.name, pdev->id, __func__);
...
static int flexcan_resume(struct platform_device *pdev)
{
	...
	dev_dbg("%s.%d: %s irq set irq wake\n", fimx6d.name, pdev->id, __func__);
